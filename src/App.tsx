...
import SatelliteAnalyze from './components/SatelliteAnalyze';
...
export default function App() {
  ...
  //   'dashboard'    — globe + ops panel (default)
  //   'fleet'        — full-screen fleet overview (roster, add/remove, telemetry)
  //   { satId: ... } — per-satellite analyze page
  const [view, setView] = useState<'dashboard' | 'fleet' | { satId: string }>('dashboard');
  ...
  async function processImageData(
    ...
    // Route this detection into the fleet alert stream so the dashboard
    // shows it on the globe + in the mission-control feed.
    const fleet = getFleet();
    const reporter = pickReportingSat(fleet, rule.rule);

    // Audit entry for EVERY inference, even LOW priority (powers the
    // Mission Log + Bandwidth panels in the menu drawer).
    auditLog.emit({
      satId: reporter.id,
      category: 'INFERENCE',
      level: rule.priority === 'CRITICAL' ? 'CRITICAL' : rule.priority === 'HIGH' ? 'WARN' : rule.priority === 'WARNING' ? 'WARN' : 'OK',
      message: `${rule.rule} → ${rule.action} · ${formatBytes(payloadBytes)} payload · ${(payload.compression_ratio * 100).toFixed(1)}% saved`,
      rawBytes,
      payloadBytes
    });

    if (rule.priority !== 'LOW' || rule.rule === 'EDGE_ACTIVE') {
      const severity =
        rule.priority === 'CRITICAL' ? 'CRITICAL' :
        rule.priority === 'HIGH' ? 'HIGH' :
        rule.priority === 'WARNING' ? 'WARNING' : 'INFO';
      fleetAlerts.emit({
        id: `${Date.now()}-${reporter.id}`,
        satId: reporter.id,
        ...
        lon: reporter.lon,
        thumbnailDataUrl,
        scoresFire: fa.scores.fire,
        scoresAnomaly: fa.scores.anomaly,
        scoresWater: fa.scores.water,
        rawBytes,
        payloadBytes
      });
    }
      ...
      {view === 'dashboard' ? (
        <>
          ...
          <main className="flex-1 min-h-0">
            <Dashboard
              onOpenSatellite={(id) => setView({ satId: id })}
              onOpenFleet={() => setView('fleet')}
            />
          </main>
        </>
      ) : view === 'fleet' ? (
